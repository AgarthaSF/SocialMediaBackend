...
func ProcessAudioMatch(c *gin.Context) (*response.AudioMatchResp, error) {
	...
	if err := models.DB.Where("matched_id = 0 and expired_time < ?",
		...
		if err != gorm.ErrRecordNotFound {
			...
			token, err := util.GenerateRtcToken(uint32(audioMatchReq.Uid), audioMatchReq.ChannelName, rtctokenbuilder.RolePublisher)
			if err != nil {
				return nil, err
			}
		...
		// generate agora application token
		token, err := util.GenerateRtcToken(uint32(audioMatchReq.Uid), channelName, rtctokenbuilder.RoleSubscriber)
		if err != nil {
			return nil, err
		}
